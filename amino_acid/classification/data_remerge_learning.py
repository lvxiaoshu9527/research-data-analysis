...
class AnalysisApp:
    ...
    def run_main_analysis(self):
        ...
        # --- [修改点] 将模型评估逻辑移至新函数 ---
        self.run_evaluation(X_train_scaled, y_train, X_test_scaled, y_test, X_train.columns, le, save_base_dir)

    def run_evaluation(self, X_train, y_train, X_test, y_test, feature_columns, le, save_base_dir):
        ...
        for model_name, model_instance in final_models.items():
            self.display_confusion_matrix(model_instance, X_test, y_test, le.classes_, model_name, save_base_dir)
            self.display_feature_importance(model_instance, feature_columns, model_name)

        if save_base_dir:
            self.export_all_reports_to_excel(all_reports_data, le.classes_, save_base_dir)
            self.export_summary_to_txt(all_reports_data, summary_stats, save_base_dir)

        best_model_name = pd.DataFrame(summary_stats).sort_values('CV_F1_Mean', ascending=False).iloc[0][
            'Model'].lower()
        self.log(f"\n根据交叉验证表现，最佳模型是: {best_model_name.upper()}")
        best_model = final_models[best_model_name]

        self.visualize_best_model(best_model_name, best_model, feature_columns, le.classes_)
        # 使用原始（未缩放）的特征列名导出模型
